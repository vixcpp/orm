...
namespace vix::orm::tools
{
    ...
    std::string MigratorCLI::getFlagValue(const std::vector<std::string> &args,
    ...
    bool MigratorCLI::hasFlag(const std::vector<std::string> &args,
                              const std::string &key)
    {
    ...
    MigratorCLI::Options MigratorCLI::parseArgsOrThrow(int argc, char **argv)
    {
        ...
        // Need at least: prog host user pass db cmd
        if (argc < 6)
